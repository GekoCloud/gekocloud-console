...
module.exports = {
  ...
  'SmartKube Settings': 'SmartKube 設置',
  ...
  CHOOSE_PROVIDER_DESC:
    'SmartKube 提供了在主流服務商中快速部署 Kubernetes 集群的方案',
  ...
  UNBIND_CLUSTER_DESC:
    '解綁集群後，SmartKube 將無法再對該集群進行管理。 解綁後，該集群内的 Kubernetes 資源不會被刪除。',
  ...
  CLUSTER_AGENT_IMPORT_TIP:
    'SmartKube 控制平面通過代理方式連接待導入集群，控制平面啟動一個公開的代理服務，待導入集群創建相應的客戶端組件連接代理服務，與控制平面之間建立一個反向代理。此種方式不需要待導入集群和控制平面在同一網路，也不要求待導入集群暴露集群的 ApiServer 地址，但會有一定的網路性能損耗</br></br>通常適用於:</br>1. 目前集群和待導入集群不在同一網路中<br/>2. 目前集群和待導入集群無法通過vpn或穿隧協定等其它技術連通所在網路<br/>3. 對集群間網路性能損耗能容忍',
  ...
  CLUSTER_KUBESPHERE_SETTINGS_DESC: '針對 SmartKube 的一些客制化設置',
