...
function transform(transformOption, importName, matches) {
    ...
    var isDefaultFunction = transformOption === 'defaultRule';

    if (/\.js$/i.test(transformOption) || isFunction || isDefaultFunction) {
        var transformFn;

        try {
            if (isFunction) transformFn = transformOption;
            else if (isDefaultFunction) transformFn = require('../lego-ui/lib/utils/importRule.js');
            else transformFn = require(transformOption);
        } catch (error) {
    ...
    return transformOption.replace(/\$\{\s?([\w\d]*)\s?\}/ig, function(str, g1) {
        ...
        return {importResultName: matches[g1], skipDefaultConversion: false };
    });
}
            ...
            ImportDeclaration: function (path, state) {
                ...
                if (hasOpts) {
                    ...
                    memberImports.forEach(function(memberImport) {
                        ...
                        var newImportSpecifier = (opts.skipDefaultConversion || replace.skipDefaultConversion)
                            ? memberImport
                            : types.importDefaultSpecifier(types.identifier(memberImport.local.name));

                        transforms.push(types.importDeclaration(
                            [newImportSpecifier],
                            types.stringLiteral(replace.importResultName)
                        ));
                    });
