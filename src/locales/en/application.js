...
export default {
  ...
  APPLICATIONS_DESC:
    'An application provides users with comprehensive business functions in one package. App templates in Smartkube are built on the Helm packaging specification. They are delivered through a unified public or private Helm repository. An application is composed of one or more Kubernetes objects including workloads, services and ingress.',
  APP_DEPLOYMENT_DESC:
  ...
  APPLICATION_TYPE_DESC:
    'Smartkube supports application deployments (based on Helm) from the App Store, as well as Application CRD.',
  ...
  FROM_APP_STORE_DESC:
    'From the official App Store of Smartkube with high-quality applications and easy deployments.',
  ...
  APP_TEMPLATES_MODAL_DESC:
    'The app template comes from the workspace and the third-party Helm app templates. It supports one click deployment and can be visualized in Smartkube to show and provide deployments and management functions.',

  TIP_APP_TYPE:
    'Smartkube supports application deployment (based on Helm) from app stores and app repositories, as well as home-made applications (Application CRD).',
  TIP_APP_GOVERNANCE:
    'Using App Governance requires creating a homebrew app and turning on service governance for each service',
  APP_REPOS_DESC:
    'The app repository comes from a third-party Helm Chart Repo, which visually displays and provides deployment and management capabilities in Smartkube. Users can quickly deploy applications with one click based on templates in the app repository.',
