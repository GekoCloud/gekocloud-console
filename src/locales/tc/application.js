...
export default {
  ...
  APPLICATION_TYPE_DESC:
    'Smartkube 支持來自於應用商店和應用倉庫的應用部署(基於 Helm)，同樣也支持自制應用 (Application CRD)。',
  ...
  'App Types': '應用的類型',
  TIP_APP_TYPE:
    'Smartkube 支持來自於應用商店和應用倉庫的應用部署(基於 Helm)，同样也支持自制應用 (Application CRD)。',
  ...
  FROM_APP_STORE_DESC:
    '來自Smartkube官方應用商店，提供高質量應用和簡易的部署方式',
  ...
  APP_TEMPLATES_MODAL_DESC:
    '應用模板來自於企業空間和第三方的Helm應用模板，支持一鍵部署並可通過視覺化的方式在Smartkube中展示並提供部署及管理的功能',
  APP_REPOS_DESC:
    '應用倉庫來自於第三方的 Helm Chart Repo，通過視覺化的方式在 Smartkube 中展示並提供部署及管理功能，用戶可以基於應用倉庫中的模板快速地一鍵部署應用。',
