...
export default {
  ...
  'Please input the kubesphere api server address of the cluster':
    '请输入待加入集群的 Smartkube API Server 地址',
  ...
  NO_CLUSTER_TIP_DESC:
    '集群是一组运行着 Kubernetes 的节点 (物理或者虚拟机)，Smartkube 的功能也依托于集群中的节点来运行',
  ADD_NEW_CLUSTER_DESC: '添加新的 Kubernetes 集群',
  CHOOSE_PROVIDER_DESC:
    'Smartkube 提供了在主流服务商中快速部署 Kubernetes 集群的方案',
  ...
  UNBIND_CLUSTER_DESC:
    '解绑集群后，Smartkube 将无法再对该集群进行管理。 解绑后，该集群内的 Kubernetes 资源不会被删除。',
  ...
  CLUSTER_API_SERVER_TITLE: '待加入集群的 Smartkube API Server',
  CLUSTER_API_SERVER_DESC: '需要添加待加入集群的 Smartkube API Server 地址',

  INPUT_KUBECONFIG: '请填写目标集群的 KubeConfig',

  CLUSTER_DIRECT_IMPORT_TIP:
    'Smartkube 多集群控制平面通过提供的 kubeconfig 来直接连接导入集群，此种方式要求当前集群能够通过 kubeconfig 中的 server 地址直接访问待导入集群。 </br></br>通常适用于:</br>1. 当前集群和待导入集群在同一内网网络中</br>2. 当前集群和待导入集群已通过 VPN 或隧道等其它技术连通所在网络</br>3. kubeconfig 的 server 地址可以通过公网访问',
  CLUSTER_AGENT_IMPORT_TIP:
    'Smartkube 控制平面通过代理方式连接待导入集群，控制平面启动一个公开的代理服务，待导入集群创建相应的客户端组件连接代理服务，与控制平面之间建立一个反向代理。此种方式不需要待导入集群和控制平面在同一网络，也不要求待导入集群暴露集群的 apiserver 地址，但会有一定的网络性能损耗。</br></br>通常适用于:</br>1. 当前集群和待导入集群不在同一网络中<br/>2. 当前集群和待导入集群无法通过 VPN 或隧道等其它技术连通所在网络<br/>3. 对集群间网络性能损耗能容忍',
