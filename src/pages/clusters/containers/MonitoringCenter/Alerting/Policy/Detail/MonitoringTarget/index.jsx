...
class MonitoringTarget extends React.Component {
  constructor(props) {
    super(props)
  ...
  renderItem(type, item) {
    if (type === 'node') {
      return <NodeItem key={item.name} data={item} />
    }
    ...
    if (type === 'workload') {
      ...
      return (
        <WorkloadItem
          key={item.name}
          prefix={`/projects/${namespace}/${workloadType}`}
          type={workloadType}
          data={item}
        />
      )
...
export default inject('rootStore')(observer(MonitoringTarget))
export const Component = MonitoringTarget
